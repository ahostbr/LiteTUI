...
def test_lifecycle_inventory_exposes_pending_shell_stop():
    from litetui.gui_rpc import active_work
    from litetui.tasks import Task, request_kill

    pending = Task("pending", "powershell", "starting", "session", 1, owner_pid=os.getpid())
    pending._pending_process_handoff = True
    uncoordinated = Task("uncoordinated", "powershell", "other", "session", 1, owner_pid=os.getpid())
    nonshell = Task("nonshell", "subagent", "agent", "session", 1, owner_pid=os.getpid())
    nonshell._pending_process_handoff = True
    foreign = Task("foreign", "powershell", "sibling", "session", 1, owner_pid=987654321)
    foreign._pending_process_handoff = True
    app = SimpleNamespace(_chat_running=lambda: False,
                          bg_tasks={t.id: t for t in (pending, uncoordinated, nonshell, foreign)}, workers=[])
    assert {entry["id"]: entry["cancellable"] for entry in active_work(app)} == {
        "pending": True, "uncoordinated": False, "nonshell": False,
    }
    assert request_kill(pending) == (True, None)
    assert "pending" not in {entry["id"] for entry in active_work(app)}
