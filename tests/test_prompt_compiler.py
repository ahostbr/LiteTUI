...
@pytest.mark.parametrize("name", ["<Ada>", "${HOME}"])
def test_user_name_placeholder_shapes_are_literal(tmp_path: Path, name: str) -> None:
    compiled = compile_prompt(
        "You are helpful.${USER_NAME_CLAUSE}", root=tmp_path, user_name=name
    )
    assert compiled == f"You are helpful. Your user's name is {name}."
