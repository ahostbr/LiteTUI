...
#: What each backend is CALLED on screen.
#:
#: ⚠️ A FOURTH LIST OF THE SAME SET, AND I AM SAYING SO RATHER THAN HIDING IT.
#: Sentinel is building a single BACKENDS registry this hour; when it lands this
#: table is one of the copies that should fold into it. Until then a missing
#: entry degrades to the backend's own `name`, which is ugly and true, rather
#: than to another backend's name, which was the bug.
_ENGINE_LABELS = {
    "llamacpp": "llama.cpp",
    "lmstudio": "LM Studio",
    "ninfer": "NInfer (NVFP4)",
...
class ModelConfigBody(Widget):
    ...
    def compose(self) -> ComposeResult:
        ...
        engine_label = _ENGINE_LABELS.get(app.backend.name, app.backend.name)
