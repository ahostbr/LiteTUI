...
#:
#: TOTAL CALLER BOUND on a _send is NOT just SEND_TIMEOUT: on timeout the
#: teardown also runs _kill_child, which does a bounded terminate-wait (5s) plus
#: a bounded kill-wait (5s). So the worst case a caller blocks on is roughly
#: SEND_TIMEOUT + 10s (~20s), not SEND_TIMEOUT alone. Anyone budgeting a caller
#: on SEND_TIMEOUT alone is off by the kill waits.
SEND_TIMEOUT = 10.0
...
class MCPServer:
    ...
    def _send(self, payload: dict, *, timeout: float = SEND_TIMEOUT) -> None:
        ...
        with self._send_lock:
            self._writer = t             # register BEFORE start, under the same lock
        t.start()
        t.join(timeout)
        if t.is_alive():
