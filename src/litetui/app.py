...
def _decode_rate(timings: object) -> float | None:
    ...
    rather than at import time. The function is llama.cpp-shaped, not
    NInfer-specific — `timings` is llama.cpp's object and both engines emit it —
    so it costs nothing on the other backends, which simply never set the field.
    """
    if timings is None:
        return None
    try:
...
#
# Sentinel's card asked for `ninfer_error_sentence` at the three
# `_ensure_chat_ready` sites. I wrote the helper, then found it had no caller —
# because the mapping ALREADY reaches the user one layer down:
# `ninfer_backend._get_json` turns the engine's `code` into a sentence AT THE
# SOURCE and raises `BackendError` carrying it, and `_plain_backend_error`
# above returns a BackendError's message verbatim (T137: "its message was
# cleaned at source").
#
...
#
# ⚠️ ONE GAP, NAMED RATHER THAN PAPERED OVER: errors raised by the OpenAI
# CLIENT during streaming are not BackendErrors and carry their body elsewhere,
# so the code table does not reach those. That is its own card — it needs the
# client's exception shape measured, not guessed.
        ...
        for _iteration in range(self.settings.tool_iterations):
            ...
            try:
                async for chunk in stream:
                    ...
                    # sites — see `decode_rate_from_timings`.
